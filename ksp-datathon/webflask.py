...
model = pickle.load(open(r'C:\Users\trilo\Downloads\webflask\model1.pkl', 'rb'))

# Load the dataset
data = pd.read_csv(r"C:\Users\trilo\Downloads\webflask\Final-dataset1.csv")
...
# Load accused data
accused_df = pd.read_csv(r"C:\Users\trilo\Downloads\webflask\AccusedData.csv")
...
def get_accused_by_location_year(location, year):
    accused_details = accused_df[(accused_df['PresentAddress'] == location) &
                                 (accused_df['Year'] == year)]
    accused_details = accused_details.dropna(subset=['Caste'])  # Remove rows with NaN values in 'Caste' column
    return accused_details
...
def predict():
    ...
    return render_template('prediction1.html', prediction=prediction, warning_image=warning_image, location=village_name, year=year)
